%Finds the distance of maximum sholl intersections along the dendrite, averages
%and runs stats (Two-Way ANOVA). Plots information as well. 
%%
[CVctrT DMctrT] = max(SIctrT');
[CVctrT DMctrN] = max(SIctrN');
[CVDT DMDT] = max(SIDT');
[CVDT DMDN] = max(SIDN');
...
[SHOLL.dmANOVA.p,~,SHOLL.dmANOVA.stats] = anovan(allValues(:), {aLevels, bLevels(:)}, 'Display', 'off');
disp('----------------Puncta Dendrite Maximum--------------');
disp('')
disp('-------------------- 2-way ANOVA  -------------------');
disp(['factor  Treatment,            p = ' num2str(SHOLL.dmANOVA.p(1))]);
disp(['factor  Location,             p = ' num2str(SHOLL.dmANOVA.p(2))]);
disp(' ');

% figure; 
% [SHOLL.dmANOVA.ttests.c SHOLL.dmANOVA.ttests.m SHOLL.dmANOVA.ttests.h SHOLL.dmANOVA.ttests.gnames] = multcompare(SHOLL.dmANOVA.stats,'Dimension', [1 2],'CType', 'bonferroni');
...
box off;
title('Distance of Maximum Dendrite Complexity');
